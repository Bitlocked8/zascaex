...
<div class="p-6">
    ...
    <!-- Información y formulario de coordenadas -->
    <div class="mb-6 bg-white p-4 rounded-lg shadow-md">
        ...
        <button id="guardar-coords" 
                class="mt-2 px-4 py-2 bg-emerald-500 text-white rounded hover:bg-emerald-600 transition">
            Guardar coordenadas
        </button>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    let lat = {{ $cliente->latitud ?? -17.393993 }};
    let lng = {{ $cliente->longitud ?? -66.170568 }};
    ...
    let marker = L.marker([lat, lng], { draggable: false }).addTo(map)
        .bindPopup(`<strong>{{ $cliente->nombre }}</strong><br>Haz clic en el mapa para cambiar coordenadas.`)
        .openPopup();
    ...
    map.on('click', function(e) {
        ...
        // Mover marcador al nuevo punto clickeado
        marker.setLatLng([lat, lng])
              .getPopup()
              .setContent(`<strong>{{ $cliente->nombre }}</strong><br>Coordenadas: ${lat.toFixed(6)}, ${lng.toFixed(6)}`)
              .openOn(map);
