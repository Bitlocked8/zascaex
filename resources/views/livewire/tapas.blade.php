    ...
    @forelse($tapas as $tapa)
      <div class="card-teal flex flex-col gap-4 p-4">
        <div class="flex flex-col gap-2">
          ...
          <p><strong>Color de tapa:</strong> {{ $tapa->color ?? 'N/A' }}</p>
          <p><strong>Tipo de tapa:</strong> {{ $tapa->tipo ?? 'N/A' }}</p>
          ...
          @if($tapa->existencias->isEmpty())
            <p class="text-gray-500 text-sm font-medium">N/A</p>
          @else
            @foreach($tapa->existencias as $existencia)
              <p class="text-cyan-700 text-sm font-medium">
        ...
        <div class="flex flex-wrap justify-center md:justify-center gap-2 border-t border-gray-200 pt-3 pb-2">
          <button wire:click="modaldetalle({{ $tapa->id }})" class="btn-cyan flex items-center gap-1 flex-shrink-0"
            title="Ver Detalles">
          ...
          <button wire:click="abrirModal('edit', {{ $tapa->id }})" class="btn-cyan flex items-center gap-1 flex-shrink-0"
            title="Editar">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none"
              stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
