                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($sucursales as $sucursal)
                                        <button type="button" wire:click="filtrarSucursalElemento({{ $sucursal->id }})"
                                            class="px-3 py-1 rounded-lg border-2 transition{{ $filtroSucursalElemento == $sucursal->id ? 'bg-cyan-600 text-white border-cyan-600' : 'border-gray-300 text-gray-700 hover:border-cyan-600 hover:text-cyan-600' }}">
                                            {{ $sucursal->nombre }}
                                        </button>
                                    @endforeach
                                </div>
                            ...
                            @if($accion === 'edit')
                                ...
                                <p
                                    ...
                                    <span class="font-medium text-u">
                                        {{ $tipo }}:
                                        {{ $as->existencia->existenciable->descripcion ?? 'Asignado #' . $asignado_id }}
                                    </span>
    ...
    @if($confirmingDeleteSopladoId)
        <div class="modal-overlay">
            <div class="modal-box">
                ...
                <div class="modal-footer">
                    <button type="button" wire:click="eliminarSopladoConfirmado" class="btn-circle btn-cyan"
                    ...
                    <button type="button" wire:click="$set('confirmingDeleteSopladoId', null)" class="btn-circle btn-cyan"
                        title="Cancelar">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M10 10l4 4m0-4l-4 4" />
