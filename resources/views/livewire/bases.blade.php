      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-teal-50 sticky top-0 z-10">
          <tr>
            ...
            <th class="px-4 py-2 text-left text-teal-700 font-semibold">Tipo de base</th>
            <th class="px-4 py-2 text-left text-teal-700 font-semibold">Capacidad (ml)</th>
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          ...
          <tr class="hover:bg-teal-50">
            <td class="px-4 py-2">{{ $base->descripcion ?? 'Sin descripción' }}</td>
            <td class="px-4 py-2">{{ $base->tipo ?? 'N/A' }}</td>
            <td class="px-4 py-2">{{ $base->capacidad ?? 'N/A' }}</td>
            <td class="px-4 py-2">
            ...
            <td class="px-4 py-2 flex justify-center gap-1">
              <button wire:click="modaldetalle({{ $base->id }})" class="btn-cyan" title="Ver Detalles">Ver más</button>
              <button wire:click="abrirModal('edit', {{ $base->id }})" class="btn-cyan" title="Editar">Editar</button>
            </td>
          </tr>
          @empty
          <tr>
            <td colspan="6" class="text-center py-4 text-gray-600">No hay bases registradas.</td>
          </tr>
          @endforelse
        </tbody>
      </table>
