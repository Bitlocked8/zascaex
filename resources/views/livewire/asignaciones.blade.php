<div class="p-2 mt-20 flex justify-center bg-white">
    <div class="w-full max-w-screen-xl">
        ...
        <div class="flex items-center gap-2 mb-4 flex-wrap">
            <input type="text" wire:model.live="searchCodigo" placeholder="Buscar por código o descripción..."
                class="input-minimal w-full sm:w-auto flex-1" />
            <button wire:click="abrirModal" class="btn-cyan flex items-center gap-1">Añadir</button>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-teal-50 sticky top-0 z-10">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left text-teal-700 font-semibold">Cantidad combinada</th>
                        <th class="px-4 py-2 text-left text-teal-700 font-semibold">Materiales asignados</th>
                        <th class="px-4 py-2 text-left text-teal-700 font-semibold">Fecha asignación</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-teal-50">
                        <td class="px-4 py-2">{{ $asignado->codigo ?? 'N/A' }}</td>
                        <td class="px-4 py-2">{{ $asignado->descripcion ?? 'N/A' }}</td>
                        <td class="px-4 py-2">
                            @if($asignado->soplados()->exists())
                            <span class="bg-emerald-100 text-emerald-700 font-bold px-2 py-1 rounded-full">Soplado</span>
                            @endif
                            @if($asignado->llenados()->exists())
                            <span class="bg-blue-100 text-blue-700 font-bold px-2 py-1 rounded-full">Llenado</span>
                            @endif
                            @if($asignado->traspasos()->exists())
                            <span class="bg-orange-100 text-orange-700 font-bold px-2 py-1 rounded-full">Traspaso</span>
                            @endif
                        </td>
                        <td class="px-4 py-2">{{ $asignado->cantidad }}</td>
                        <td class="px-4 py-2">
                            ...
                            <div class="text-sm text-slate-600">
                                ...
                                (Restante: {{ $reposicion->pivot->cantidad }} / Original: {{ $reposicion->pivot->cantidad_original }})
                            </div>
                            @endforeach
                        </td>
                        <td class="px-4 py-2">{{ \Carbon\Carbon::parse($asignado->fecha)->format('d/m/Y H:i') }}</td>
                        <td class="px-4 py-2 flex justify-center gap-1">
                            ...
                            @if($cantidadPivote > 0)
                            <button wire:click="abrirModal('edit', {{ $asignado->id }})" class="btn-cyan" title="Editar">Editar</button>
                            <button wire:click="confirmarEliminarAsignacion({{ $asignado->id }})" class="btn-cyan" title="Eliminar">Eliminar</button>
                            @endif
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="7" class="text-center py-4 text-gray-600">No hay asignaciones registradas.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
