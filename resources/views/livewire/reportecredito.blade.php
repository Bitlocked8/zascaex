          ...
          <tbody>
            ...
            <tr class="color-bg border border-slate-200">
              <td class="px-6 py-4 p-text text-left">
                <div class="font-semibold">{{ $cliente->empresa ? $cliente->empresa : $cliente->nombre }} <br> {{ $cliente->celular }} {{ $cliente->telefono ? ' - '. $cliente->telefono : '' }}</div>
              </td>
              <td class="px-6 py-4 p-text text-left">
                ...
                <div class="mb-2">
                  ...
                  <span
                      class="font-semibold {{ 
                          now()->gt(Carbon\Carbon::parse($venta->fechaMaxima)) ? 'text-red-600' : 
                          (now()->diffInDays(Carbon\Carbon::parse($venta->fechaMaxima), false) <= 7 ? 'text-yellow-600' : 'text-green-600')
                      }}"
                  >
                      Fecha Máxima: {{ $venta->fechaMaxima }}
                  </span>
                  <span>
                    Monto Pendiente: Bs
